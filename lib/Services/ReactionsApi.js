...
var create = function create() {
  ...
  var getReactions = function getReactions(q) {
    return api.get('reactions', q);
  };

  var addReaction = function addReaction(q) {
    var accessToken = (0, _booshReactAuth.getAccessToken)();
    if (accessToken) api.setHeader('Authorization', 'Bearer ' + accessToken);
    return api.post('reactions', q);
  };

  var removeReaction = function removeReaction(id) {
    var accessToken = (0, _booshReactAuth.getAccessToken)();
    if (accessToken) api.setHeader('Authorization', 'Bearer ' + accessToken);
